...
def train(train_data, dev_data, model, model_par, criterion, optimizer):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        train_loss = run_epoch(train_data, model_par,
                               MultiGPULossCompute(model.lora_transformer.generator, criterion, config.device_id, optimizer))
        logging.info("Epoch: {}, loss: {}".format(epoch, train_loss))
        # 模型验证
        model.eval()
        dev_loss = run_epoch(dev_data, model_par,
                             MultiGPULossCompute(model.lora_transformer.generator, criterion, config.device_id, None))
        bleu_score = evaluate(dev_data, model.lora_transformer)
        logging.info('Epoch: {}, Dev loss: {}, Bleu Score: {}'.format(epoch, dev_loss, bleu_score))
        Notification(content=f'''epoch:{epoch}, train_loss:{train_loss}, 
                                 dev_loss:{dev_loss}, bleu_score:{bleu_score}'''
                     , summary=f"autodl ted epoch{epoch} done")
        
        # 如果当前epoch的模型在dev集上的loss优于之前记录的最优loss则保存当前模型，并更新最优loss值
        if bleu_score > best_bleu_score:
            torch.save(model.state_dict(), config.lora_model_path)
...
def test(data, model, criterion):
    with torch.no_grad():
        # 加载模型
        model.load_state_dict(torch.load(config.lora_model_path))
        model_par = torch.nn.DataParallel(model)
        model.eval()
        # 开始预测
        test_loss = run_epoch(data, model_par,
                              MultiGPULossCompute(model.lora_transformer.generator, criterion, config.device_id, None))
        bleu_score = evaluate(data, model.lora_transformer, 'test')
