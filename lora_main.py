...
from lora_model import LoRA_Transformer
from lora_train import train, test, translate
...
def get_std_opt(model):
    ...
    return NoamOpt(model.lora_transformer.src_embed[0].d_model, 1, 10000,
...
def run():
    ...
    train_dataset = MTDataset(config.lora_train_data_path)
    dev_dataset = MTDataset(config.lora_dev_data_path)
    test_dataset = MTDataset(config.lora_test_data_path)
    ...
    model = LoRA_Transformer(model, config.r, config.d_model)
    ...
    test_loss, bleu_score =  test(test_dataloader, model, criterion)
    Notification(content=f'''测试结束, {config.lora_model_path}在{config.lora_test_data_path}上
                 test loss = {test_loss}, 
                 bleu= {bleu_score}''', 
                 summary="laptop测试结束")
...
if __name__ == "__main__":
    ...
    Notification(content="开始训练", summary="autodl ted开始训练了")
    run()
